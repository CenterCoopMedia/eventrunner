...
| Header | `config/theme.header`; `cmsPages.layout.header`; `resolveHeader` |
...
|---|---|---|---|
| Site header | `Header` | The site identity at the top of every page, in one of four treatments: `standard`, `masthead`, `compact`, `minimal`. | The theme names the default and a page may override it. Type and rules only — never a hero banner, never a photo behind the name. The identity repeats on every page, so it is NEVER a heading: each page owns its own `h1`. An unrecognized value renders the base rather than no header at all. |
...
- **A client sets one colour, and the supporting colours are derived from it**: the darker and lighter steps, the three rule weights and the control boundary (ink mixed into the ground at fixed shares), and the whole dark palette (authored from the light one, never inverted). Semantic colours come from the site style and are not part of the brand decision — a client's colour does not get to redefine what "warning" means. A raw per-mode override is an EXCEPTIONAL control for a brand guide that names an exact value: every use takes one token out of the derivation permanently, so it stops moving when the brand colour moves.
...
- The admin has its own fixed palette: the `admin-*` tokens (design brief §5.2). The admin obeys `data-mode` and ignores `data-theme`, so the `admin-*` blocks are emitted once per mode and never once per theme. **The position marker is derived from the admin's own ink against the admin ground — it is not a client setting.** There is no marker colour in the Branding tab, which means there is no value to get wrong, no legibility floor to explain, and no fallback state to report.
