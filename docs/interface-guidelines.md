...
- **Where the navigation sits is a SITE setting**, `config/theme.navPlacement` (`top` or `side`). A reader who meets a top nav on the home page and a rail on the schedule has been handed two sites, so one choice covers every page and the page editor does not offer a per-page one. `layout.navPlacement` is still accepted on write and still read by the shell as a page's own fallback, because deployments made before the move set it per page and nothing may restyle their pages on upgrade. `side` moves the same navigation to the leading edge at wide viewports: same landmark, same items, same place in the document, and below `lg` it is the top nav again.
