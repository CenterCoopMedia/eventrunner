...
- **A client sets one color, and the supporting colors are derived from it**: The darker and lighter steps, the three rule weights and the control boundary (ink mixed into the ground at fixed shares), and the whole dark palette (authored from the light one, never inverted). Semantic colors come from the site style and are not part of the brand decision — a client's color does not get to redefine what "warning" means. A raw per-mode override is an EXCEPTIONAL control for a brand guide that names an exact value: every use takes one token out of the derivation permanently, so it stops moving when the brand color moves. No second field feeds the site palette. The one other colour a client can set is `adminAccent` — **Admin marker colour** — and it is admin-only: the position marker and the mark on the page-header rule, blank for the admin's own ink. It derives nothing, so it cannot move a site token. It carries the legibility floor the brief sets (§5.2): a value that fails against `--admin-ground` sends both slots back to `--admin-ink`, and the editor states the ratio it measured and what it fell back to. The value is stored exactly as typed and never silently clamped, and the admin never renders an invisible marker.
