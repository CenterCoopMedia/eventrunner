...
- **Stored mail renders only in the preview frame.** A stored email body reaches the screen only through `admin/emailPreview.js` `buildPreviewDoc`, as the `srcdoc` of an `<iframe sandbox="">` whose document opens with a content policy that blocks script and every fetch. Never through `innerHTML`, and never with a sandbox token added.
