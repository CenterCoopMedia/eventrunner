...
This record replaces that one decision. It does not touch the "ICS export + calendar links" row: the `.ics` download and the Google and Outlook links stay behind the `icsExport` flag, as they are today, and they are the fallback here. Every other decision in ADR 0001 stands.
...
- It requires three other flags: `schedule` and `sessionBookmarks`, because the control lives on the My schedule page and that page needs both, and `icsExport`, because the file export is the fallback and the fallback must exist. The control renders only when all four are true. The admin editor lets each flag change on its own, so the reconciling branch makes the editor refuse `calendarSync` on while any of the three is off, and names the one that is off.
- Flag off: no control, no Google script, and no OAuth request. The page is what it is today. No new local storage key is written. A calendar id stored while the flag was on stays in the browser, unused; it is not deleted, because deleting it would make a second calendar if the flag comes back on.
- Flag on before the client finishes the setup in decision 5: the control renders and offers the sync. What the attendee sees depends on which step is missing. While the client's app is in testing status, Google refuses the grant for anyone who is not a listed test user, and the control shows its "not granted" state. If the grant succeeds but the Calendar API is not enabled in the client's project, Google accepts the grant and refuses the first Calendar call; the control shows "not set up", a state of its own that says the event has not finished its calendar setup. In both states the `.ics` download sits beside the control, there is no broken button, and nothing is written. The checklist says to turn the flag on last. If a client turns it on early, this is what an attendee sees.
...
- Each event carries two private extended properties: a fixed marker that says the app wrote it, and the session id. The session id is the only key. A pass asks Google only for events that carry the marker (the list call's private extended property filter), so an event the attendee adds to the calendar by hand is never fetched, changed, or deleted. The implementation on `main` lists every event in the calendar and ignores the untagged ones; the reconciling branch moves it to the filter.

How updates and removals follow bookmarks:

- The unit of work is a pass. A pass lists the events in the app's calendar that carry the marker, deletes the ones whose session is no longer bookmarked, updates the ones whose session still is, and creates the rest. A pass is idempotent.
...
- Calendar API not enabled, or the project's Calendar access otherwise not set up (Google answers the Calendar call with a refusal that names the project, not the attendee): the pass stops before any write, and the control shows "not set up" with the `.ics` download. The attendee cannot fix this, so the state says the event has not finished its calendar setup and does not ask for a new grant.
...
- [ ] Privacy page. The client's privacy policy states that, at the attendee's request, saved sessions are written to a calendar the app creates in the Google account the attendee chooses, what is sent, and how to revoke. The legal review sign-off in §3 happens after this edit, not before.
...
- [ ] Admin panel → [Settings → Features](../ADMIN_GUIDE.md#settings-features): confirm `schedule`, `sessionBookmarks`, and `icsExport` are on, then turn `calendarSync` on.
...
- ADR 0001 §9 loses one row. Every other row holds. The `.ics` export and calendar links stay behind `icsExport`, and every deployment that turns `calendarSync` on has them.
...
- The implementation on `main` is reconciled in the branch that follows acceptance: it sends the event's time zone on each event, renders only when its three prerequisite flags are on, makes the admin editor refuse `calendarSync` without them, lists only marked events, and matches the states in decision 4, the "not set up" state included. Tests sit next to the change. No rule changes, so no rules test is added. The control stays brand neutral, with a keyboard path and visible focus.
