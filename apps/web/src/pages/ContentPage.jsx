        ...
        filteredSections.map(({ section, blocks }, index) => (
          ...
          >
            {/* A section whose label repeats the page title, or whose id
                marks it as an intro/header stand-in (isTitleRepeatingSection
                above), keeps it for screen readers only — and with no
                visible heading there is no section boundary to draw either.
                Data-driven, never render position: a filter can put a
                different section at index 0, and that section still needs
                its real, visible heading. tabIndex={-1} on both heading
                forms lets the section index (above) move focus here without
                pulling either into the tab order (interface guidelines:
                Accessibility — only tabindex 0 and -1). */}
