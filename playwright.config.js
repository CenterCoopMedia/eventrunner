...
// The dev server the webServer block below launches has to bind the SAME
// host/port that baseURL/webServer.url (both derived from APP_URL) probe
// and navigate to — an E2E_APP_URL override that changed one without the
// other would start Vite on 127.0.0.1:5173 regardless while Playwright
// polled a different address, timing out even though a server is running.
// Parsed once here so overriding APP_URL is a one-variable change, not a
// two-variable one that silently drifts apart.
...
export default defineConfig({
  ...
  webServer: {
    // --host pins the bind address explicitly. Without it, Vite 5 binds
    // "localhost", and on ubuntu-latest runners Node's DNS resolution of
    // "localhost" can land on ::1 while Playwright's own webServer probe
    ...
    // regardless. --host/--port are derived from APP_URL (above) rather
