...
| branch 16 | `joe/confident-gates-gy99ip-16-timeline` | #194 | c3 in `/home/user/wt-int7`, with the History list on the eager path (spec); the feedback dialog loads on demand to make room. Branch 15 merged in `58376b9` |
...
State at 09:10 UTC: branches 12 to 16 are pushed or pushing; branch 16 is the top. Next on the stack: c6 (#197; its review is `wf_daa53d9c-da6`; its fix round is running in `/home/user/wt-c6`; it also changes `adminSource.js` so admin listeners wait for the server) as branch 17, then c7 (#198; its review confirmed two minor findings, and the director accepted the +71 raw and +37 gzip admin chunk overage; c7 was cut from c5's pre-fix tip `78fac23`, and its SectionHead action slot must merge with c3's) as branch 18. The demo's initial chunk is the tightest budget: 290,653 of 292,000 gzip bytes on branch 16.
