...
test('each style keeps its own conviction in its recommended configuration', () => {
  ...
  // The traced line down the forward column, which is Atlas's alone: every
  // other style holds a zero width there and paints nothing. The transfer
  // line is NOT on this list any more — it is not preset-gated, because a
  // surveyed walking time is a fact and a fact five styles hide is a fact
  // withheld from the reader who needed it most.
  assert.notEqual(resolvePresetTokens(configuration('atlas'))['--schedule-trace-width'], '0');
